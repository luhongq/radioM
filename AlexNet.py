...
class ImageOnlyModel(nn.Module):
    ...
    def __init__(self, image_feature_dim=128, final_output_dim=1):
        super(ImageOnlyModel, self).__init__()

        # ResNet 图像特征分支
        resnet = models.resnet18(pretrained=True)
        resnet.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.feature_extractor = nn.Sequential(*list(resnet.children())[:-1])  # 去掉最后的 FC 层
        self.fc = nn.Sequential(
            nn.Linear(512, image_feature_dim),
            nn.ReLU(),
            nn.Linear(image_feature_dim, final_output_dim)
        )

    def forward(self, image):
        x = self.feature_extractor(image)  # 提取图像特征
        x = torch.flatten(x, 1)  # 展平特征维度
        output = self.fc(x)  # 回归输出
        return output
...
class NumericOnlyModel(nn.Module):
    ...
    仅使用 MLP 数值分支
    """
    def __init__(self, input_dim=4, hidden_dims=[32, 16], output_dim=64, final_output_dim=1):
        super(NumericOnlyModel, self).__init__()
    ...
    def forward(self, features):
        x = self.feature_branch(features)  # 数值特征提取
        output = self.fc(x)                # 回归输出
        return output
...
class TransformerBranch(nn.Module):
    def __init__(self, input_dim=4, output_dim=64):
        ...
        self.transformer = nn.TransformerEncoderLayer(
            d_model=32, nhead=4, dim_feedforward=128, dropout=0.3
        )
        self.fc1 = nn.Linear(input_dim, 32)
        self.fc2 = nn.Linear(32, output_dim)

    def forward(self, x):
        x = self.fc1(x).unsqueeze(0)  # (batch, seq_len, dim)
...
# 替换 ResNet 为 MobileNet
class MobileNetFullModel(nn.Module):
    def __init__(self, image_feature_dim=128, num_feature_dim=64, final_output_dim=1):
        super(MobileNetFullModel, self).__init__()
        self.image_branch = MobileNetBranch(output_dim=image_feature_dim)  # MobileNet 分支
        self.feature_branch = MLPBranch(input_dim=4, output_dim=num_feature_dim)  # MLP 分支
        self.fc = nn.Sequential(
            nn.Linear(image_feature_dim + num_feature_dim, 64),
            nn.ReLU(),
            nn.Linear(64, final_output_dim)
    ...
    def forward(self, image, features):
        image_out = self.image_branch(image)  # 图像特征提取
        feature_out = self.feature_branch(features)  # 数值特征提取
        combined = torch.cat([image_out, feature_out], dim=1)  # 特征拼接
        output = self.fc(combined)  # 回归输出
        return output
# 替换 MLP 为 Transformer
class TransformerFullModel(nn.Module):
    def __init__(self, image_feature_dim=128, num_feature_dim=64, final_output_dim=1):
        super(TransformerFullModel, self).__init__()
        self.image_branch = ResNetBranch(output_dim=image_feature_dim)  # ResNet 分支
        self.feature_branch = TransformerBranch(input_dim=4, output_dim=num_feature_dim)  # Transformer 分支
        self.fc = nn.Sequential(
            nn.Linear(image_feature_dim + num_feature_dim, 64),
            nn.ReLU(),
            nn.Linear(64, final_output_dim)
...
class MobileNetTransformerModel(nn.Module):
    """
    图像分支：MobileNet
    数值分支：Transformer
    """
    ...
    def __init__(self, image_feature_dim=128, num_feature_dim=64, final_output_dim=1):
        super(MobileNetTransformerModel, self).__init__()

        # 图像分支：MobileNet
        self.image_branch = MobileNetBranch(output_dim=image_feature_dim)

        # 数值分支：Transformer
        self.feature_branch = TransformerBranch(input_dim=4, output_dim=num_feature_dim)
    ...
    def forward(self, image, features):
        image_out = self.image_branch(image)  # MobileNet图像特征提取
        feature_out = self.feature_branch(features)  # Transformer数值特征提取

        # 特征拼接
        combined = torch.cat([image_out, feature_out], dim=1)
